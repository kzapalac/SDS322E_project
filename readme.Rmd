...
df %>% filter(!is.na(inOrOut), !is.na(PT_avg)) %>% ggplot(aes(y=PT_avg, x = priming_received)) + geom_violin() + facet_wrap(~inOrOut) +
  ggtitle("Ease of Perspective Taking for In-Group Vs. Out-Group Stranger Based on Priming Received") + ylab("perspective taking scores") + xlab("priming received") 
df %>% filter(!is.na(inOrOut), !is.na(PT_avg)) %>% ggplot(aes(y=PT_avg, x = priming_received)) + geom_boxplot() + facet_wrap(~inOrOut) +
  ggtitle("Ease of Perspective Taking for In-Group Vs. Out-Group Stranger Based on Priming Received") + ylab("perspective taking tcores") + xlab("priming received") 
df %>% filter(!is.na(inOrOut), !is.na(PT_avg)) %>% ggplot(aes(y=PT_avg, x = priming_received)) + geom_bar(stat='summary') + facet_wrap(~inOrOut) +
  geom_errorbar(stat='summary', width = 0.3) +
  ggtitle("Average of Perspective Taking Scores for In-Group Vs. Out-Group Strangers") + ylab("perspective taking score average") + xlab("priming received")
