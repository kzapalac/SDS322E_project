...
### Difference in Responses Depending on Priming Received
```{r}
...
## Additional EDA
**From here on, we created additional graphs to explore the data and determined what we wanted to include in the final report. there are not explanations for each graph and corresponding hypotheses like above.**
...
ggplot(df, aes(x = age, y = dg)) + geom_point() + geom_smooth(method = 'lm') + 
  ggtitle('Altruism\'s Relationship with Age') +
  ylab('altruism') +
  theme(plot.title = element_text(hjust = 0.5))
ggplot(df, aes(x = age, y = PT_avg)) + geom_point() + geom_smooth(method = 'lm')+ 
  ggtitle('Perspective Taking\'s Relationship with Age') +
...
```
It would appear that the out-group is slightly older just by chance. Age doesn't appear to have much of a relationship with any of our variables. 
